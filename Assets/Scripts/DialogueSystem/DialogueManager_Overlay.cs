...
public class DialogueManagerOverlay : MonoBehaviour
{
    ...
    private IEnumerator TypeText(string text, TMP_FontAsset currentFont)
    {
        ...
        {
            if (currentFont.name == "Font Diner Boss SDF" || currentFont.name == "Font Mayas Mom v2 SDF" || currentFont.name == "Font Maya SDF" || currentFont.name == "Font Cop SDF" || currentFont.name == "Font Kid Customer SDF" || currentFont.name == "Font Arcade Manager Bold SDF" || currentFont.name == "LiberationSans SDF - Fallback")
            {
                UsingUpperCase = true;
